...
let showFavoritesOnly = false; // Variável para controlar se deve exibir apenas favoritos
...
function filterGames() {
    ...
    showFavoritesOnly = formData.has('favorites'); // Verifica se o checkbox de favoritos está marcado

    filteredGames = games.filter(game => {
        const gameStores = Object.keys(game.prices);
        const matchesStore = selectedStores.length === 0 || selectedStores.some(store => gameStores.includes(store));
        ...
        // Verifica se o jogo é favorito, se a opção de mostrar apenas favoritos estiver selecionada
        const isFavorite = JSON.parse(localStorage.getItem('favoriteGames') || '[]').includes(game.title);
...
function displayGames(page) {
    ...
    gamesToDisplay.forEach((game, index) => {
        ...
        minPriceFixedLabel.textContent = `Menor Preço : R$ ${minPrice.toFixed(2).replace('.', ',')} (${minDay} dias atrás)`;
        ...
        const prices = [];
        for (let i = 1; i <= 40; i++) {
            prices.push(parseFloat(game.priceHistory[`DaysAgo${i}`].replace('R$', '').replace(',', '.')));
        }
...
function toggleFavorite(icon) {
    ...
    if (isFavorite) {
        ...
        if (index > -1) {
            ...
            document.getElementById('message-text').innerText = 'Jogo removido dos favoritos!';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const searchQuery = urlParams.get('nome-jogo');
    
    if (searchQuery) {
        const searchResults = games.filter(game => game.title.toLowerCase().includes(searchQuery.toLowerCase()));
